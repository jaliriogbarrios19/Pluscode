...
- **FILE SIZE LIMIT (intransgredible):** No source file shall exceed 300 lines. Before writing or editing ANY file, check its line count. If an edit would push a file past 300 lines, STOP and inform the user — propose a split strategy. Never autonomously split existing +300-line files; only flag them for the user to decide. This applies to all source code files (.ts, .tsx, .js, .jsx, .css, etc.). Configuration files (package.json, tsconfig.json, etc.), generated files (main.js bundle), and i18n/data dictionaries are exempt unless the user says otherwise.
...
| Skill maintenance, archiving, patching skills | skill-curator |
| Go tests, Bubbletea TUI testing | go-testing (in `optional-skills/`) |
| Model selection, routing, MiMo, Deepseek, qué modelo usar | model-router |
| Agno agent, agent runtime, web search, code analysis, build agent | agno-agent |

Load skills BEFORE writing code. Apply ALL patterns. Multiple skills can apply simultaneously.

**Optional skills** live in `optional-skills/` and are NOT auto-loaded. To activate one, the user must explicitly install it or the curator must propose it. Currently: `go-testing`, `presentations`, `judgment-day`.
