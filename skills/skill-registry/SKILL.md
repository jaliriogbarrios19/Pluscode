---
name: skill-registry
description: "Trigger: update skills, skill registry, actualizar skills, skill changes. Create or update the project skill registry."
license: MIT
metadata:
  author: gentleman-programming
  version: "1.1"
  tags: [skills, registry, indexing, discovery, delegation]
...
`capture_prompt: false` is required because the skill registry is an automated artifact. Set it when the Engram tool schema supports it; if an older schema does not expose the field, omit it rather than failing.
...
- [ ] `.atl/skill-registry.md` written with correct structure
- [ ] Engram saved (if available)
- [ ] All SDD skills (`sdd-*`, `_shared`, `skill-registry`) excluded from scan
- [ ] Compact rules are 5-15 lines per skill — concise and actionable
- [ ] All convention index files found and their referenced paths extracted
- [ ] Trigger table complete with all discovered skills
