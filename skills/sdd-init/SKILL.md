...
metadata:
  ...
  version: "3.0"
  tags: [sdd, initialization, setup, persistence]
  related_skills: []
---
...
- Project root path (from orchestrator context or user)
- Persistence mode: `engram`, `openspec`, `hybrid`, or `none`
- Optional: explicit Strict TDD preference, existing `openspec/` directory

## Execution and Persistence Contract

| Artifact | Persistence |
|----------|-------------|
| Project context (stack, conventions, architecture) | `mem_save` type=`config` topic_key=`sdd/{project}/context` |
| Testing capabilities | `mem_save` type=`config` topic_key=`sdd/{project}/testing-capabilities` |
| Skill registry (`.atl/skill-registry.md`) | Write file + `mem_save` type=`config` topic_key=`skill-registry` (capture_prompt: false) |
| Mode bootstrap (`openspec/` if applicable) | Write files to `openspec/` |
