...
description: "Trigger: implementation, commit splitting, chained PRs, keeping tests with code. Plan commits as reviewable work units."
license: Apache-2.0
metadata:
  author: gentleman-programming
  version: "1.1"
  tags: [commits, work-units, splitting, review, chained-prs]
  related_skills: [branch-pr, chained-pr]
---
...
Don't use for:
- PR creation mechanics — use `branch-pr` skill
- Splitting already-large PRs — use `chained-pr` skill first
...
1. **Committing by file type.** `add models`, `add services`, `add tests` as separate commits — none works alone and tests are divorced from the code they verify.
2. **Forgetting docs.** Docs belong with the user-visible change, not in a separate commit. A reviewer can't verify intent without them.
3. **Unclear commit messages.** Messages that describe file changes ("updated auth.py") instead of outcomes ("fix(auth): handle expired tokens gracefully").
4. **Ignoring SDD workload forecasts.** When SDD warns of >400 lines, ignoring the signal and creating a monolithic PR.

---

## Verification Checklist

- [ ] Each commit has one clear purpose (behavior, fix, migration, or docs unit)
- [ ] Tests are in the same commit as the behavior they verify
- [ ] Docs are in the same commit as the user-visible change they explain
- [ ] Commit message describes the outcome, not the file list
- [ ] Repo still makes sense after applying each commit individually
- [ ] Rollback of any commit is reasonable without reverting unrelated work
- [ ] If approaching 400 lines, commits are grouped into chained PR candidates
