---
name: branch-pr
description: "Trigger: creating, opening, preparing PRs, pull requests. Create Gentle AI pull requests with issue-first checks."
license: Apache-2.0
metadata:
  author: gentleman-programming
  version: "2.1"
  tags: [github, pull-requests, automation, branch, conventional-commits]
  related_skills: [chained-pr, work-unit-commits, issue-creation]
---
...
- User mentions "PR", "pull request", "create PR", "open PR"
...
1. **Forgetting to link an issue.** PRs without `Closes #N` are blocked by CI. Always verify the issue has `status:approved` before opening.
2. **Wrong branch name format.** Branches must match `type/description`. Spaces, uppercase, or special chars will fail validation.
3. **Multiple type labels.** Exactly ONE `type:*` label is enforced. Adding two causes CI failure.
4. **Squashing during active review.** Don't squash commits while review is ongoing — it destroys incremental context.
