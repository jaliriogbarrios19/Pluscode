---
name: issue-creation
description: "Trigger: creating GitHub issues, bug reports, feature requests. Create Gentle AI issues with issue-first checks."
license: Apache-2.0
metadata:
  author: gentleman-programming
  version: "1.1"
  tags: [github, issues, bug-report, feature-request, triage]
  related_skills: [branch-pr]
---
...
- User mentions "issue", "bug report", "feature request", "create issue"

Don't use for:
- Questions or discussions — use [Discussions](https://github.com/Gentleman-Programming/agent-teams-lite/discussions) instead
- PR creation — use `branch-pr` skill
...
1. **Not searching for duplicates first.** Always check existing issues before creating a new one.
2. **Missing required fields.** Blank/incomplete issues are blocked. Fill ALL required fields.
3. **Using issues for questions.** Questions go to Discussions — maintainers will redirect and close.
4. **Opening a PR without approval.** PRs referencing unapproved issues are blocked by CI.
...
- [ ] Correct template selected (Bug Report or Feature Request)
- [ ] All required fields filled in
- [ ] Pre-flight checkboxes checked
- [ ] Title follows conventional format (type(scope): description)
- [ ] Issue references link correctly if related to other issues
